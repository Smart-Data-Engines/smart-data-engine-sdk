...
| The schema does not match the map | a **missing** column is refused at `ensure_schema`, with the difference printed. An **extra** column is logged and accepted, because you may have added it outside this library and the map says nothing about it | nothing | nothing: the refusal happens at startup rather than at the first write that needs the column | `test_schema_statements.py` |
