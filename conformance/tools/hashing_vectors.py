...
# Twelve hex characters, i.e. 48 bits - the same constant the libraries use, restated rather than
# imported, so that --verify needs nothing but the standard library and openssl. That matters: it is
# what CI runs, and a check that needs the library installed to confirm the library is right has
# quietly become a check of something else.
DIGEST_CHARS = 12
...
def name_map_document(names: Any, salt: bytes) -> dict[str, Any]:
...
def build(case: Path) -> None:
    # Imported here rather than at module scope so that --verify runs on a bare checkout.
    sys.path.insert(0, str(ROOT / "python" / "src"))
    import sde
    from sde.canonical import canonical_bytes
    from sde.hashing import DIGEST_CHARS as library_digest_chars
    from sde.hashing import hash_identifiers
    from sde.testing.loader import model_from_neutral

    if library_digest_chars != DIGEST_CHARS:
        raise SystemExit(
            f"the library truncates digests to {library_digest_chars} characters, this generator to "
            f"{DIGEST_CHARS}. One of them changed without the other, and section 2a says which is "
            "right."
        )
    ...
    write_json(case / "names.json", name_map_document(names, salt))
...
def verify(case: Path) -> None:
    """Re-derive every committed digest with openssl and compare. Writes nothing.

    This is what CI runs. It is the difference between "the vectors match this library" and "the
    vectors match the format document", and it costs a second.
    """
    salt = bytes.fromhex((case / "salt.hex").read_text().strip())
    committed = json.loads((case / "names.json").read_text(encoding="utf-8"))

    checks: list[tuple[str, str, list[str], str]] = []
    for entity, hashed in committed["entities"].items():
        checks.append((entity, "e_", [entity], hashed))
    for entity, mapping in committed["fields"].items():
        for field, hashed in mapping.items():
            checks.append((f"{entity}.{field}", "f_", [entity, field], hashed))
    for entity, mapping in committed["relations"].items():
        for relation, hashed in mapping.items():
            checks.append((f"{entity}.{relation}", "r_", [entity, relation], hashed))

    for label, prefix, parts, committed_digest in checks:
        independent = openssl_digest(salt, prefix, parts)
        if independent != committed_digest:
            raise SystemExit(
                f"{case.name}: {label} is committed as {committed_digest}, openssl derives "
                f"{independent}. Either the vector was generated by an implementation that builds "
                "the HMAC message differently from section 2a, or section 2a changed without the "
                "vector being regenerated."
            )
    print(f"  {case.name}: {len(checks)} digests confirmed against openssl")
...
def main() -> None:
    ...
    parser.add_argument(
        "--verify",
        action="store_true",
        help="re-derive the committed digests with openssl and compare; writes nothing",
    )
    arguments = parser.parse_args()
    cases = sorted(p for p in VECTORS.iterdir() if p.is_dir())
    if not cases:
        raise SystemExit("no hashing vectors found, so this checked nothing")
    ...
    if not arguments.i_am_changing_the_contract:
        raise SystemExit(
            ...
            "--i-am-changing-the-contract if that is what you are doing, or --verify to check the "
            "committed digests without touching them."
        )
    for case in cases:
        build(case)
