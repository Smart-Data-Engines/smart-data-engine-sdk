"""Stage and switch storage for a separate TypeScript application process."""
...
is_stage = request["plan"]["kind"] == "sde-stage"
loader = sde.load_staging_plan if is_stage else sde.load_cutover_plan
plan = loader(
...
try:
    ...
    if isinstance(plan, sde.StagingPlan):
        executor.enroll(plan.as_record()["current"])
        result = executor.stage(plan)
    else:
        result = executor.execute(plan)
    print(json.dumps(result.as_record()), flush=True)
finally:
